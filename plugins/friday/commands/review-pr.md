...
| 3 | 자동화 봇이 만든 PR | "자동화 PR은 리뷰 대상이 아닙니다" |
| 4 | 내가 리뷰할 필요 없음 | "이미 리뷰를 완료했고 재요청이 없습니다" |
| 5 | FE 변경 없고 라벨 없음 | "FE 리뷰 대상이 아닙니다" |
...
1단계에서 획득한 `author` 필드 확인:
- `querypie-automation`: 조기 종료 (체리픽, 자동화 작업용 PR)
- 그 외: 리뷰 진행

```markdown
## ℹ️ 리뷰 대상 아님

이 PR은 **querypie-automation**이 생성한 자동화 PR입니다.
```
